...
export const GpsExtractionStep: FC = () => {
  ...
  }, [photos, setGpsData, setLoading, addGpsError, clearGpsErrors, setTrajectoryPath])

  // Auto-extract GPS data when component mounts
  useEffect(() => {
    if (photos.length > 0 && gpsData.length === 0 && !loading) {
      handleExtractGps()
    }
  }, []) // Only run on mount
  ...
  const canProceed = stats.successful >= 2 && !loading
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-3 gap-2 md:gap-4">
          ...
          <div className="p-3 md:p-4 bg-green-50 border border-green-200 rounded-lg">
            <div className="flex items-center gap-1 md:gap-2 mb-1">
              <CheckCircle2 className="size-3 md:size-4 text-green-600" />
          ...
          <div className="p-3 md:p-4 bg-red-50 border border-red-200 rounded-lg">
            <div className="flex items-center gap-1 md:gap-2 mb-1">
              <AlertCircle className="size-3 md:size-4 text-red-600" />
      ...
      {!loading && stats.successful < 2 && gpsData.length > 0 && (
        <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <div className="flex gap-3">
            <AlertCircle className="size-5 text-yellow-600 shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-medium text-yellow-900 mb-1">
                {t('workspace.gps.insufficientData', { defaultValue: 'Insufficient GPS Data' })}
              </p>
              <p className="text-sm text-yellow-700">
                {t('workspace.gps.insufficientDataMessage', { defaultValue: 'At least 2 photos with valid GPS coordinates are required to create a trajectory. Please upload more photos with location data.' })}
              </p>
            </div>
          </div>
        </div>
